...
```{r}
Numbers<-Complete_data4%>%select(Asset.Manager.AUM.Dollars.Million,Asset.Manager.Inter.AUM.Dollars.Million,AUM.Dollars.Million,Audience.Size,Opened,Clicked,Bounced,Unsubscribed)

summary_statistics<-
  list(
    "Asset Manager AUM ($M)"=
      list(
        "mean (sd)" = ~qwraps2::mean_sd(Asset.Manager.AUM.Dollars.Million, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Asset.Manager.AUM.Dollars.Million, na_rm=TRUE),
        "min" = ~min(Asset.Manager.AUM.Dollars.Million, na_rm=TRUE),
        "max" = ~max(Asset.Manager.AUM.Dollars.Million, na_rm=TRUE),
        "Missing" = ~sum(is.na(Asset.Manager.AUM.Dollars.Million))
      ),
    "Asset Manager Intermediary AUM ($M)"=
    list(
        "mean (sd)" = ~qwraps2::mean_sd(Asset.Manager.Inter.AUM.Dollars.Million, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Asset.Manager.Inter.AUM.Dollars.Million, na_rm=TRUE),
        "min" = ~min(Asset.Manager.Inter.AUM.Dollars.Million, na_rm=TRUE),
        "max" = ~max(Asset.Manager.Inter.AUM.Dollars.Million, na_rm=TRUE),
        "Missing" = ~sum(is.na(Asset.Manager.Inter.AUM.Dollars.Million))
      ),
    "AUM ($m)"=
      list(
        "mean (sd)" = ~qwraps2::mean_sd(AUM.Dollars.Million, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(AUM.Dollars.Million, na_rm=TRUE),
        "min" = ~min(AUM.Dollars.Million, na_rm=TRUE),
        "max" = ~max(AUM.Dollars.Million, na_rm=TRUE),
        "Missing" = ~sum(is.na(AUM.Dollars.Million))
      ),
    "Audience Size"=
      list(
        "mean (sd)" = ~qwraps2::mean_sd(Audience.Size, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Audience.Size, na_rm=TRUE),
        "min" = ~min(Audience.Size, na_rm=TRUE),
        "max" = ~max(Audience.Size, na_rm=TRUE),
        "Missing" = ~sum(is.na(Audience.Size))
      ),
    "Open Rate"=
            list(
        "mean (sd)" = ~qwraps2::mean_sd(Opened, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Opened, na_rm=TRUE),
        "min" = ~min(Opened, na_rm=TRUE),
        "max" = ~max(Opened, na_rm=TRUE),
        "Missing" = ~sum(is.na(Opened))
      ),
    "Click Rate"=
          list(
        "mean (sd)" = ~qwraps2::mean_sd(Clicked, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Clicked, na_rm=TRUE),
        "min" = ~min(Clicked, na_rm=TRUE),
        "max" = ~max(Clicked, na_rm=TRUE),
        "Missing" = ~sum(is.na(Clicked))
      ),
    "Bounced Rate"=
      list(
        "mean (sd)" = ~qwraps2::mean_sd(Bounced, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Bounced, na_rm=TRUE),
        "min" = ~min(Bounced, na_rm=TRUE),
        "max" = ~max(Bounced, na_rm=TRUE),
        "Missing" = ~sum(is.na(Bounced))
      ),
    "Unsubscribe Rate"=
            list(
        "mean (sd)" = ~qwraps2::mean_sd(Unsubscribed, na_rm=TRUE),
        "median (Q1, Q3)"= ~qwraps2::median_iqr(Unsubscribed, na_rm=TRUE),
        "min" = ~min(Unsubscribed, na_rm=TRUE),
        "max" = ~max(Unsubscribed, na_rm=TRUE),
        "Missing" = ~sum(is.na(Unsubscribed))))
summary_table(Complete_data4,summary_statistics)
...
df_split<-initial_split(Complete_data_SD_2,prop=.8,strata = Opened)
