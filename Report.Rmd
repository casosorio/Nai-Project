...
dt = sort(sample(nrow(Complete_data), nrow(Complete_data)*.7))
train<-Complete_data[dt,]
test<-Complete_data[-dt,]

k_fold<-vfold_cv(train)

model_rec<-recipe(Opened~date+month+Reference.Name+From+Mail.Type+Product.Name+Subscription+Blast.Message+Distribution.Partner.Specific+Preferred.Partner+Producers.Only+Prospects.Only, train) %>%
